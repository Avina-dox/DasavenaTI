...
const badge = (status) => {
  switch (status) {
    case "in_stock":
      return "bg-green-700/30 text-green-200 border-green-400/30";
    case "assigned":
      return "bg-blue-700/30 text-blue-200 border-blue-400/30";
    case "repair":
      return "bg-yellow-700/30 text-yellow-200 border-yellow-400/30";
    case "retired":
      return "bg-red-700/30 text-red-200 border-red-400/30";
    default:
      return "bg-gray-700/30 text-gray-200 border-gray-400/30";
  }
};

// solo para darle formateo breve al teléfono
const fmtPhone = (v) => {
  const s = (v || "").replace(/[^\d+]/g, "");
  // +52 55 1234 5678 (simple)
  return s.startsWith("+")
  ...
  return (
    <section className="min-h-screen bg-gradient-to-br rounded-2xl from-[#484f61] via-[#1a1e2a] to-[#111318] py-10 px-4 md:px-8 lg:px-16">
      ...
      <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-8">
        <div>
          <h1 className="text-4xl font-extrabold text-[#E9C16C] tracking-widest drop-shadow mb-1">
            Activos
          </h1>
          <p className="text-[#E9C16C]/70 text-sm">
      ...
      <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4">
        {loading && (
          <div className="col-span-full flex justify-center items-center py-16">
            <span className="text-[#E9C16C]/70 animate-pulse text-lg">
              Cargando…
            </span>
          </div>
        )}
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <span className="font-mono text-base font-bold text-[#E9C16C]">
                    {a.asset_tag}
                  </span>
                  <span
                ...
                <div className="space-y-1 text-sm">
                  ...
                  {/* ——— SOLO Teléfonos ——— */}
                  {isPhone && (
                    <div className="mt-2 rounded-xl bg-[#0d111a]/40 border border-white/10 p-2">
                      ...
                      {!!a.is_unlocked && (
                        <span className="mt-2 inline-flex items-center gap-1 rounded-full border border-emerald-400/30 bg-emerald-700/20 px-2 py-0.5 text-[11px] text-emerald-200">
                          ● Libre
                        </span>
                      )}
                    </div>
                  ...
                  <div className="mt-2 rounded-xl bg-white/5 border border-white/10 p-2">
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div className="text-xs text-[#E9C16C]/60">
                        Dep.: 10% anual{" "}
                        {dep
                          ? `• ${Math.floor(dep.years)}a ${Math.round(
                              (dep.years % 1) * 12
                            )}m`
                          : ""}
                      </div>
