...
import { useColorMode } from "../components/ThemeProvider"; // ajusta la ruta si cambiaste carpeta
...
// Colores de gráficas según el tema MUI
const useChartColors = () => {
  const theme = useTheme();
  const grid = theme.palette.mode === "dark" ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.08)";
  const border = theme.palette.divider;
  const label = theme.palette.text.primary;
  return { grid, border, label, isDark: theme.palette.mode === "dark" };
};
...
export default function Inicio() {
  ...
  const pieData = useMemo(() => {
    ...
    return {
      ...
      datasets: [{ data: Object.values(counts), backgroundColor: COLORS, borderWidth: 2 }]
    };
  }, [rows]);
      ...
      <Card
        sx={{
          ...
          background: theme.palette.mode === "dark"
            ? "linear-gradient(135deg,#2d1b3d 0%,#2a2e45 100%)"
            : "linear-gradient(135deg,#e9eefc 0%,#eaf7f0 100%)",
          ...
          <Stack direction="row" alignItems="center" spacing={2} justifyContent="space-between">
            ...
            <Stack direction={{ xs: "column", sm: "row" }} spacing={1} alignItems={{ xs: "stretch", sm: "center" }}>
              ...
              <Stack direction="row" spacing={1} alignItems="center">
                <IconButton onClick={fetchData} sx={{ color: theme.palette.mode === "dark" ? "#fff" : theme.palette.text.primary }} aria-label="refrescar">
                  <RefreshIcon />
                </IconButton>

                {/* Toggle claro/oscuro + modo sistema */}
                <IconButton onClick={toggle} sx={{ color: theme.palette.mode === "dark" ? "#fff" : theme.palette.text.primary }} title="Alternar claro/oscuro">
                  {theme.palette.mode === "dark" ? <LightModeIcon /> : <DarkModeIcon />}
                </IconButton>
                <Chip
                  size="small"
                  onClick={() => setMode(mode === "system" ? "light" : "system")}
                  icon={<MonitorIcon sx={{ color: theme.palette.mode === "dark" ? "#fff" : theme.palette.text.primary }} />}
                  label={mode === "system" ? "Sistema" : "Fijar tema"}
                  sx={{
                ...
                {loading
                  ...
                  : <Box sx={{ height: 240, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Pie
                      ...
                      options={{
                        plugins: {
                          legend: { labels: { color: chartColors.label } },
                          tooltip: { enabled: true }
                        },
                        elements: { arc: { borderColor: chartColors.border, borderWidth: 2 } }
                      }}
                    />
                  </Box>
                }
                  ...
                  : <Box sx={{ height: 260 }}>
                    <Bar
                      ...
                      options={{
                        ...
                        scales: {
                          x: { grid: { display: false }, ticks: { color: chartColors.label, font: { size: 12 } } },
                          y: { beginAtZero: true, grid: { color: chartColors.grid }, ticks: { color: chartColors.label, stepSize: 1 } }
                        }
                      }}
                    />
                  </Box>
                ...
                <Grid container spacing={2}>
                  ...
                  {!loading && porUsuario.map(([user, assets]) => (
                    <Grid item xs={12} sm={6} md={4} lg={3} key={user}>
                      <Card sx={{
                        p: 2,
                        borderLeft: `6px solid ${user === "Sin asignar" ? "#fbbf24" : "#6b8afd"}`
                      }}>
